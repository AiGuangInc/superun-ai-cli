...
发布时在 PR 中同时提高 `package.json` 和 `package-lock.json` 的版本，并将标题设为 `release: v<版本号>`，在正文提供 `## Release notes`。PR 的构建门禁会检查版本、类型、构建和打包。合并到 `main` 后，[GitHub Actions](.github/workflows/publish-npm.yml) 使用 npm trusted publishing 发布到 `latest`，随后验证公开安装和 CLI 版本。不要再运行旧的 `npm run release` 手工发包流程。
...
在可识别会话归属的 Codex、WorkBuddy / CodeBuddy 环境中，CLI 会记住当前会话正在创作的一个项目。新建项目会替换原绑定；显式项目操作直接使用传入的 ID，并在后台尝试保存绑定。列出项目、查询详情或等待任务不会改变绑定。
...
遇到修复方式等确认卡时，先展示对应的问题说明，再逐题展示问题及完整选项。如果展示管线省略了提问前的正文，CLI 从同一条消息的服务端解析快照恢复该提问对应的可见说明；不读取模型推理，不用旧报告替代当前确认依据。
...
CLI 每次执行业务命令前都查询 npm `latest`。版本不一致时，使用当前 Node 运行 npm，在独立的新目录安装目标版本；核对包版本并实际启动离线 `version` 验证模块与依赖后，原子切换默认入口，再继续原业务命令。后续调用及离线 `version` 都使用已切换的版本。普通终端、WorkBuddy 的 Node 环境以及不同 CLI 安装各自维护入口，用户无需判断目录或手动升级。版本查询、安装或验证失败时停止业务，不回退旧版本继续请求；已有安装与入口保留。帮助、`version` 和 `auth logout` 可离线使用。
