...
async function verifiedEntry(directory: string, version: string): Promise<string | undefined> {
...
export async function versionGate(
  ...
  // 已安装的目标版本直接复用，避免每次执行都创建、删除更新锁。
  let entry = await verifiedEntry(join(root, required), required);
  if (!entry)
    entry = await withUpdateLock(async () => {
      source = await latestVersion();
      required = source.version;
      const destination = join(root, required);
      // 等锁期间其他进程可能已完成安装，必须再次检查。
      const cached = await verifiedEntry(destination, required);
      ...
      try {
        ...
        if (!(await verifiedEntry(staging, required)))
